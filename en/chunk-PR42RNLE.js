import{a as m,b as d,c as p}from"./chunk-WIHELTOI.js";import{a as l,b as c,c as u}from"./chunk-76M2OJZG.js";import{E as o,I as n,_ as r}from"./chunk-VKZYAJA6.js";var v=`Tu es un assistant sp\xE9cialis\xE9 en reconnaissance alimentaire.
...
R\xE9ponds UNIQUEMENT en JSON valide, sans texte avant/apr\xE8s :
{
  ...
  "notes": null
}`,h=`Tu es un assistant de saisie de sympt\xF4mes digestifs.
...
}`,P=`Tu es un assistant de saisie de m\xE9dicaments et compl\xE9ments alimentaires.
...
}`,y=class a{_settingsService=n(l);_openAi=n(u);_anthropic=n(m);_gemini=n(d);_ollama=n(p);_analyzing=r(!1);analyzing=this._analyzing.asReadonly();get _activeProvider(){let e=this._settingsService.getSelectedProvider(),i={openai:this._openAi,anthropic:this._anthropic,gemini:this._gemini,ollama:this._ollama}[e];if(!i)throw new c(`Provider "${e}" non disponible`,e);return i}async recognizeFood(e,t){this._analyzing.set(!0);try{let i=await this._activeProvider.analyzeImage(e,v,t);try{let s=i.match(/\{[\s\S]*\}/);return s?JSON.parse(s[0]):{foods:[],uncertain:[]}}catch{return{foods:[],uncertain:[]}}}finally{this._analyzing.set(!1)}}async analyzeFodmap(e){this._analyzing.set(!0);try{let t=`Aliments \xE0 analyser : ${e.join(", ")}`,i=await this._activeProvider.complete(t,_);try{let s=i.match(/\{[\s\S]*\}/);return s?JSON.parse(s[0]):{foods:[],advice:i,globalLevel:"low",globalScore:0}}catch{return{foods:[],advice:i,globalLevel:"low",globalScore:0}}}finally{this._analyzing.set(!1)}}async parseVoiceTranscript(e,t){this._analyzing.set(!0);let i={food:f,symptom:h,medication:P};try{return await this._activeProvider.complete(e,i[t])}finally{this._analyzing.set(!1)}}async analyzeCorrelations(e){this._analyzing.set(!0);try{let t=`Donn\xE9es fournies :
